...
class SingleModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            #################################################################3
            self.rerain_loss = torch.nn.MSELoss()
            ##################################################################
    ...
    def predict(self):
        ...
        # print(np.transpose(self.real_A.data[0].cpu().float().numpy(),(1,2,0))[:2][:2][:])
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A, _= self.netG_A.forward(self.real_A, self.real_A_gray)
        else:
            self.fake_B = self.netG_A.forward(self.real_A, self.real_A_gray)
        # self.rec_A = self.netG_B.forward(self.fake_B)
    ...
    def forward(self):
        ...
        if self.opt.input_linear:
            self.real_A = (self.real_A - torch.min(self.real_A))/(torch.max(self.real_A) - torch.min(self.real_A))
        if self.opt.skip == 1:
            self.fake_B, self.latent_real_A , self.rerain_image = self.netG_A.forward(self.real_img, self.real_A_gray)
        else:
            self.fake_B , self.rerain_image = self.netG_A.forward(self.real_img, self.real_A_gray)
        if self.opt.patchD:
    ...
    def backward_G(self, epoch):
        ...
        ####################################################################################
        self.loss_G = 0.8 *self.loss_G + self.rerain_loss(self.rerain_image,self.real_img)
        ############################################################################################
