...
class Unet_resize_conv(nn.Module):
    ...
    def forward(self, input, gray):
        ...
        # mask = Variable(torch.ones(batch_size, 3, row, col)) / 2.
        # h = Variable(torch.zeros(batch_size, 32, row, col))
        # c = Variable(torch.zeros(batch_size, 32, row, col))
        ############################use_GPU#########################################################
        mask = Variable(torch.ones(batch_size, 3, row, col)).cuda() / 2.
        ...
        if self.opt.use_norm == 1:
            if self.opt.self_attention:
                ...
                x = self.bn1_1(self.LReLU1_1(self.conv1_1(torch.cat((input, mask), 1))))
               # x = self.bn1_1(self.LReLU1_1(self.conv1_1(input)))
            ...
            if self.opt.linear:
                output = output/torch.max(torch.abs(output))
        ################################################################
        re_rain_image = output + mask
        ################################################################
        output = pad_tensor_back(output, pad_left, pad_right, pad_top, pad_bottom)
        latent = pad_tensor_back(latent, pad_left, pad_right, pad_top, pad_bottom)
        ####################################################################################################33
        re_rain_image = pad_tensor_back(re_rain_image, pad_left, pad_right, pad_top, pad_bottom)
        ################################################################################################################
        ...
        if self.skip:
            return output, latent,re_rain_image
        else:
            return output, re_rain_image
